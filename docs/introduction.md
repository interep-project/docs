...
In addition, users, based on their reputation score, can join **groups** ([Semaphore](https://semaphore.appliedzkp.org/) groups) anonymously so that external services can use InterRep to verify, for example, the authenticity of their users while guaranteeing their privacy.
